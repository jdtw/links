...
func TestImportFromCopiesEntriesVerbatim(t *testing.T) {
	ctx := context.Background()
	src := NewMemStore()

	// A link whose stored RequiredPaths must be preserved rather than
	// recomputed during the copy.
	uris := map[string]string{
		"plain": "http://example.com/plain",
		"subst": "http://example.com/{1}/{0}",
	}
	for k, uri := range uris {
		if _, err := src.Put(ctx, k, &pb.Link{Uri: uri}); err != nil {
			t.Fatalf("seeding %s failed: %v", k, err)
		}
	}

	dst := newTestSQLiteStore(t)
	n, err := ImportFrom(ctx, dst, src)
	if err != nil {
		t.Fatalf("ImportFrom failed: %v", err)
	}
	if n != len(uris) {
		t.Errorf("ImportFrom copied %d entries, want %d", n, len(uris))
	}

	for k, wantURI := range uris {
		le, err := dst.Get(ctx, k)
		if err != nil {
			t.Fatalf("Get(%s) failed: %v", k, err)
		}
		if le == nil {
			t.Errorf("Get(%s) = nil, want the imported entry", k)
			continue
		}
		if le.Link.GetUri() != wantURI {
			t.Errorf("Get(%s) URI = %q, want %q", k, le.Link.GetUri(), wantURI)
		}
		srcLE, err := src.Get(ctx, k)
		if err != nil {
			t.Fatalf("src.Get(%s) failed: %v", k, err)
		}
		if le.RequiredPaths != srcLE.RequiredPaths {
			t.Errorf("Get(%s) RequiredPaths = %d, want %d", k, le.RequiredPaths, srcLE.RequiredPaths)
		}
	}
}
...
func TestImportFromIsIdempotent(t *testing.T) {
	ctx := context.Background()
	src := NewMemStore()
	if _, err := src.Put(ctx, "key", &pb.Link{Uri: "http://example.com/one"}); err != nil {
		t.Fatalf("seeding failed: %v", err)
	}

	dst := newTestSQLiteStore(t)
	for i := 0; i < 2; i++ {
		if _, err := ImportFrom(ctx, dst, src); err != nil {
			t.Fatalf("ImportFrom run %d failed: %v", i+1, err)
		}
	}

	count := 0
	if err := dst.Visit(ctx, func(string, *pb.LinkEntry) { count++ }); err != nil {
		t.Fatalf("Visit failed: %v", err)
	}
	if count != 1 {
		t.Errorf("after two imports the store holds %d entries, want 1", count)
	}
}
